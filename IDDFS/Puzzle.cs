class Puzzle
{
    ...
    public Puzzle(Puzzle? parents, List<int[]> tiles)
    {
        this._parent =parents;
        this._tiles = tiles;
    }
    public Puzzle(Puzzle? parents)
    {
        _tiles = this.GenArr();
        this._parent = parents;
    }
    ...
    public Puzzle Move(int f_x,int f_y,int to_x,int to_y)
    {  
        Puzzle puzzle = new(this);
        Copy(puzzle,this);
        (puzzle.tiles[f_x][f_y],puzzle.tiles[to_x][to_y])= (puzzle.tiles[to_x][to_y] , puzzle.tiles[f_x][f_y]);
        return puzzle;

    }
    public void Copy(Puzzle p1 , Puzzle p2 )
    {
